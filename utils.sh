...
JVM_OPTS="-XX:+UseParallelGC -XX:+UseStringDeduplication -Xmx2G"
...
# Combine multiple patch JARs
combine_patch_jars(){
  local output_jar=$1
  shift
  local -a patch_jars=("$@")
  pr "Combining ${#patch_jars[@]} patch sources"
  # Create temporary working directory
  local tmp_dir
  tmp_dir=$(mktemp -d -p "$TEMP_DIR")
  
  # Extract each patch JAR and combine contents
  for ((i=0; i<${#patch_jars[@]}; i++)); do
    local jar="${patch_jars[$i]}"
    local extract_dir="${tmp_dir}/patches_$i"
    mkdir -p "$extract_dir"
    if ! unzip -q -o "$jar" -d "$extract_dir" &>/dev/null; then
      epr "Failed to extract $jar"
      rm -rf "$tmp_dir"
      return 1
    fi
  done
  
  # Create combined directory and merge all contents
  local combined_dir="${tmp_dir}/combined"
  mkdir -p "$combined_dir"
  find "$tmp_dir" -maxdepth 1 -mindepth 1 -type d -name "patches_*" -print0 | 
    xargs -0 -I{} cp -rf "{}/"* "$combined_dir/" 2>/dev/null || true
  
  # Create combined JAR
  (cd "$combined_dir" && zip -q -r "$output_jar" .)
  local ret=$?
...
# Get patches from multiple sources
get_multi_source_patches(){
  local table=$1 patch_sources_cfg=$2
  local -a cli_jars=() patch_jars=() patch_sources=()
  
  # Parse patch sources array
  patch_sources_cfg=${patch_sources_cfg#[}
  patch_sources_cfg=${patch_sources_cfg%]}
  IFS=', ' read -ra patch_sources <<<"${patch_sources_cfg//\"/}"
  
  if ((${#patch_sources[@]} == 0)); then
    epr "No valid patch sources found"
    return 1
  fi
  
  pr "Using patch sources: ${patch_sources[*]}"
  
  # Process all patch sources
  for src in "${patch_sources[@]}"; do
    local src_config
    src_config=$(toml_get_table "PatchSources.$src")
    
    if [[ -z "$src_config" ]]; then
      epr "Patch source '$src' not defined in PatchSources section"
      continue
    fi
    
    local src_source src_cli_source src_version src_cli_version
    src_source=$(toml_get "$src_config" "source") || continue
    src_cli_source=$(toml_get "$src_config" "cli-source") || continue
    src_version=$(toml_get "$src_config" "version") || src_version=$DEF_PATCHES_VER
    src_cli_version=$(toml_get "$src_config" "cli-version") || src_cli_version=$DEF_CLI_VER
    
    # Get prebuilts for this source
    local src_prebuilts
    if ! src_prebuilts=$(get_rv_prebuilts "$src_cli_source" "$src_cli_version" "$src_source" "$src_version"); then
      epr "Failed to get prebuilts for patch source '$src'"
      continue
    fi
    
    local src_cli_jar src_patch_jar
    read -r src_cli_jar src_patch_jar <<<"$src_prebuilts"
    
    cli_jars+=("$src_cli_jar")
    patch_jars+=("$src_patch_jar")
  done
  
  # Return if no patch JARs found
  if ((${#patch_jars[@]} == 0)); then
    return 1
  fi
  
  # Set primary CLI jar
  rv_cli_jar="${cli_jars[0]}"
  
  # If only one patch source, use it directly
  if ((${#patch_jars[@]} == 1)); then
    rv_patches_jar="${patch_jars[0]}"
    return 0
  fi
  
  # Combine patches from multiple sources
  local combined_jar="${TEMP_DIR}/combined-${table//[^a-zA-Z0-9_-]/-}-$(date +%s).jar"
  if ! combine_patch_jars "$combined_jar" "${patch_jars[@]}"; then
    epr "Failed to combine patches"
    return 1
  fi
  
  # Use combined patches
  rv_patches_jar="$combined_jar"
  return 0
}

# Apply AAPT2 optimization
optimize_apk(){
  local input_apk=$1 output_apk=$2 config=$3
  local optimize languages densities
  optimize=$(toml_get "$config" "optimize-apk") || optimize="false"
  ...
  languages=$(toml_get "$config" "optimize-languages") || languages="en"
  densities=$(toml_get "$config" "optimize-densities") || densities="nodpi,anydpi,xxhdpi"
  
  pr "Optimizing APK with AAPT2: languages=$languages, densities=$densities"
  
  # Create temp file for output
  local tmp_out
  tmp_out=$(mktemp -p "$TEMP_DIR" "${input_apk##*/}.opt.XXXXXX.apk")
  
  # Run AAPT2 optimization
  if ! "$AAPT2" optimize --target-densities "$densities" -c "$languages" -o "$tmp_out" "$input_apk" &>/dev/null; then
    epr "AAPT2 optimization failed"
    cp "$input_apk" "$output_apk"
    rm -f "$tmp_out"
    return 1
  fi
  ...
  elif [[ -f "$ANDROID_HOME/build-tools/*/zipalign" ]]; then
    zipalign_cmd=$(find "$ANDROID_HOME/build-tools/" -name "zipalign" -type f | sort -V | tail -n 1)
  else
    # No zipalign, just use AAPT2 output
    mv "$tmp_out" "$output_apk"
    return 0
  fi
  
  # Zipalign if available
  local do_zipalign
  do_zipalign=$(toml_get "$config" "zipalign") || do_zipalign="false"
  if [[ "$do_zipalign" == "true" ]]; then
    local aligned_out
    aligned_out=$(mktemp -p "$TEMP_DIR" "${input_apk##*/}.aligned.XXXXXX.apk")
    pr "Aligning APK with zipalign"
    if ! "$zipalign_cmd" -p -f 4 "$tmp_out" "$aligned_out" &>/dev/null; then
      epr "Zipalign failed, using unaligned APK"
      mv "$tmp_out" "$output_apk"
    else
      mv "$aligned_out" "$output_apk"
    fi
    rm -f "$tmp_out" "$aligned_out" 2>/dev/null || true
  else
    mv "$tmp_out" "$output_apk"
  fi
  return 0
...
get_patch_last_supported_ver(){
  ...
  if ! op=$(java $JVM_OPTS -jar "$rv_cli_jar" list-versions "$rv_patches_jar" -f "$pkg_name" 2>&1 | tail -n +3 | awk '{$1=$1}1'); then
...
merge_splits(){
  ...
  if ! OP=$(java $JVM_OPTS -jar "$TEMP_DIR/apkeditor.jar" merge -i "${bundle}" -o "${bundle}.mzip" -clean-meta -f 2>&1); then
...
get_uptodown_pkg_name(){ $HTMLQ --text "tr.full:nth-child(1) > td:nth-child(3)" <<<"$__UPTODOWN_RESP__"; }
...
# Enhanced patch_apk with optimized JVM settings
patch_apk(){
  local stock_input=$1 patched_apk=$2 patcher_args=$3 rv_cli_jar=$4 rv_patches_jar=$5
  
  # Get JVM settings from config
  local jvm_opts
  jvm_opts=$(toml_get "$main_config_t" jvm-flags) || jvm_opts="$JVM_OPTS"
  
  # Always use AAPT2 for consistency
  local cmd="env -u GITHUB_REPOSITORY java $jvm_opts -jar $rv_cli_jar patch $stock_input --purge -o $patched_apk -p $rv_patches_jar --keystore=ks.keystore \
--keystore-entry-password=r4nD0M.paS4W0rD --keystore-password=r4nD0M.paS4W0rD --signer=krvstek --keystore-entry-alias=krvstek --custom-aapt2-binary=${AAPT2} $patcher_args"
...
check_sig(){
  ...
  if grep -q "$pkg_name" sig.txt; then
    sig=$(java $JVM_OPTS -jar "$APKSIGNER" verify --print-certs "$file" | grep ^Signer | grep SHA-256 | tail -1 | awk '{print $NF}')
    grep -qFx "$sig $pkg_name" sig.txt
  fi
}

# Enhanced build_rv with multi-source patching and optimization
build_rv(){
  ...
  if [[ "${NORB:-}" != true || ! -f "$patched_apk" ]]; then
    if ! patch_apk "$stock_apk" "$patched_apk" "${patcher_args[*]}" "$rv_cli_jar" "$rv_patches_jar"; then
