...
declare -F set_prebuilts &>/dev/null || set_prebuilts(){
  ...
  # Default JVM_OPTS; allow JAVA_OPTS to seed it (config.toml can override in build.sh)
...
toml_prep(){
  ...
  if ! _toml_to_json_with_tq "$file" "$out"; then
    _toml_to_json_with_python "$file" "$out" || { epr "TOML parser unavailable (tq or python3+tomllib)"; return 1; }
  fi
  export TOML_JSON="$out"
  [[ -s "$TOML_JSON" ]]
}
toml_get_table_main(){ echo "."; }
toml_get_table(){ local name=$1; printf '.%s' "${name//./\\.}"; }
